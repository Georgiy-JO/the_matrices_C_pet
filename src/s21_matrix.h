...
#define PRECISION \
  (double)7  // how many decimal places is taken into acount for comperison
#define EPSILON \
  (double)1e-7  // precision for comparing with zero (ensues from PRECISION)
...
int s21_create_matrix(int rows, int columns,
                      matrix_t *result);  // not sure if tested enough
void s21_remove_matrix(matrix_t *A);      // not sure if tested enough
...
int s21_if_matrix_rows_legit(
    const matrix_t *matr);  // not sure if tested enough
int s21_if_matrix_legit_complex(
    const matrix_t *matr);  // not sure if tested enough

int s21_if_double_legit(double a);  // not sure if tested enough
...
int s21_matrix_rebuilder(int rows, int columns, matrix_t *result,
                         const double *array);  // not sure if tested enough
...
int s21_eq_matrix(matrix_t *A,
                  matrix_t *B);  // input errors was not tested fully
