...
// can be checked only with VALGRIND
...
int s21_filling_matrix(matrix_t *mtrx, const double *array) {
  if (!s21_if_matrix_legit(mtrx) || array == NULL) return INPUT_ERR;
...
int s21_sum_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  ...
  if (!flag)
    flag = (s21_matrix_size_compare(A, result)) ? SUCCESS_CALC : CALC_ERR;
  ////////////////////////////
  /* more complex variant in which result matrix can be rebuild if it has wrong
  size (can be brought to other calculation functions) -- need to be adjusted
  if(!flag){
    flag=(s21_matrix_size_compare(A,result))?SUCCESS_CALC:CALC_ERR;
    if(!flag && (!s21_if_matrix_legit(result) ||
  !s21_if_matrix_rows_legit(result))) flag = s21_recreate_matrix(A->rows,
  A->columns, result);
  }
  */
  ////////////////////////////
...
int s21_mult_number(matrix_t *A, double number, matrix_t *result) {
  if (!s21_if_matrix_legit_complex(A) || !s21_if_matrix_legit(result) ||
      !s21_if_matrix_rows_legit(result))
    return INPUT_ERR;
...
int s21_mult_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  ...
  if (!flag)
    flag = (result->rows == A->rows && result->columns == B->columns)
               ? SUCCESS_CALC
               : CALC_ERR;
  ////////////////////////////
  if (!flag) {
...
int s21_transpose(matrix_t *A, matrix_t *result) {
  if (!s21_if_matrix_legit(A) || !s21_if_matrix_rows_legit(A) ||
      !s21_if_matrix_legit(result) || !s21_if_matrix_rows_legit(result))
    return INPUT_ERR;
  int flag = (result->rows == A->columns && result->columns == A->rows)
                 ? SUCCESS_CALC
                 : CALC_ERR;
  ////////////////////////////
  if (!flag) {
...
int s21_inverse_matrix(matrix_t *A, matrix_t *result) {
  ...
  ////////////////////////////
  if (!flag) {
    matrix_t temp_1, temp_2;
    s21_create_matrix(A->rows, A->columns, &temp_1);
    s21_create_matrix(A->rows, A->columns, &temp_2);
    flag = s21_calc_complements(A, &temp_1);
    flag =
        (!flag && !s21_transpose(&temp_1, &temp_2)) ? SUCCESS_CALC : CALC_ERR;
    det = 1 / det;
    flag = (!flag && !s21_mult_number(&temp_2, det, result)) ? SUCCESS_CALC
                                                             : CALC_ERR;
...
array){ if(!s21_if_matrix_legit(mtrx)|| array==NULL) return INPUT_ERR; int
flag=SUCCESS_CALC; for(int i=0,k=0;flag==SUCCESS_CALC && i<mtrx->rows;i++){
    for(int j=0;flag==SUCCESS_CALC && j<mtrx->columns;j++,k++){
      if(s21_if_double_legit(array[k]))
        mtrx->matrix[i][j]=array[k];
      else
        flag=INPUT_ERR;
    }
...
/* Removed functions
//-->Next 3 functions were removed because trunc() fucked me up somehow
// (ノ ◑‿◑)ノ (I'm comparing 2 doubles)
int s21_double_compare(double a, double b){
  if(isnan(a) || isnan(b)) return COMP_ERR;
  double a_int=s21_my_trunc(a), b_int=s21_my_trunc(b);
  int flag=0;
  flag= s21_double_int_compare(a_int,b_int);
  if( flag==EQUAL){
    a_int= s21_take_decimal_places(a, a_int);
    b_int= s21_take_decimal_places(b, b_int);
    flag= s21_double_int_compare(a_int,b_int);
  }
  return flag;
}
// (ノ ◑‿◑)ノ (I'm comparing 2 intager parts of double)
//never been checked independently
int s21_double_int_compare(double a_int, double b_int){
  int flag=0;
  if(a_int>b_int) flag=BIGGER;
  else if (a_int<b_int) flag=SMALLER;
  else flag=EQUAL;
  return flag;
}
// (ノ ◑‿◑)ノ (I'm returning decimal part of a double number according to
PRECISION (input: double in the integer part of it))
//never been checked independently
double s21_take_decimal_places(double num, double num_int){
  return trunc((num-num_int)*pow(10.0,PRECISION));
}
*/
